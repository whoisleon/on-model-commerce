...
function aip_reii_confirmation_intro_v0551( $order_id ) {
	...
	<section class="aip-confirmation-hero" aria-labelledby="aip-confirmation-title">
		...
		<a class="aip-confirmation-again" href="<?php echo esc_url( home_url( '/style-by-reii/#submit-project' ) ); ?>" target="_top">Create another REii video</a>
	</section>
	<?php
}
...
function aip_reii_checkout_theme_css_v0551() {
	...
	body.woocommerce-checkout.aip-embedded-checkout .woocommerce-form-coupon-toggle,body.woocommerce-checkout.aip-embedded-checkout .wc-block-components-totals-coupon{border-bottom:1px solid var(--aip-line)!important;border-top:1px solid var(--aip-line)!important;color:#625b68!important;font-size:10px!important;padding:15px 2px!important}
	...
	body.woocommerce-checkout.aip-checkout-complete .aip-confirmation-again{border-bottom:1px solid #9f89f0;color:#fff!important;display:inline-block;font-size:9px;font-weight:800;letter-spacing:.08em;padding-bottom:4px;text-decoration:none!important;text-transform:uppercase}
...
function aip_reii_embedded_checkout_compat_styles() {
	...
	wp_register_style( 'aip-reii-embedded-compat', false, array(), '0.5.51' );
...
// Run once more after every plugin's init callback. This makes the current
// handlers authoritative even if an older duplicate registered itself later.
add_action( 'wp_loaded', 'aip_reii_register_current_intake_capture_v0549', PHP_INT_MAX );
}
...
final class AIP_On_Model_Commerce_GitHub {
	...
	public static function passwordless_delivery_request() {
		...
		foreach ( $delivery_addons as $addon_slug => $addon ) :
			$addon_url = add_query_arg( array( 'aip_offer' => $addon_slug, 'source_order' => $order->get_id() ), home_url( '/style-by-reii/' ) ) . '#submit-project';
		?>
