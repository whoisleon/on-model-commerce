...
// Keep the published order page editable; the legacy theme restores old pricing.
add_action( 'init', function () {
	remove_action( 'init', 'uncode_child_restore_reii_portal_clean' );
}, 0 );
...
function aip_reii_is_portal_request_v0596() {
	if ( function_exists( 'is_page' ) && is_page( array( 'style-by-reii', 'on-model-content' ) ) ) {
		return true;
	}
...
function aip_reii_filter_document_title_v0596( $title ) {
	if ( aip_reii_is_portal_request_v0596() ) {
		return 'REii — AI Influencer UGC Studio';
	}
	return $title;
}
add_filter( 'pre_get_document_title', 'aip_reii_filter_document_title_v0596', 999 );
...
function aip_reii_embedded_checkout_compat_styles() {
	...
	wp_register_style( 'aip-reii-embedded-compat', false, array(), '0.6.03' );
...
function aip_reii_prepare_native_checkout_v0554() {
	...
	if ( 'amazon-storefront' === $storefront_check || '1' === (string) ( $_POST['aip-addon-storefront'] ?? '' ) ) {
		if ( 'Amazon link / ASIN' === $method && ! empty( $reference ) ) {
	...
	$coupon_code = sanitize_text_field( wp_unslash( $_POST['coupon-code'] ?? '' ) );
	$offer       = aip_reii_stripe_offer_v0559( $addon );

	if ( $coupon_code ) {
		if ( ! class_exists( 'WC_Coupon' ) ) {
			aip_reii_native_checkout_error_v0554( 'Coupon verification is temporarily unavailable.' );
		}
		$coupon = new WC_Coupon( $coupon_code );
		if ( ! $coupon->get_id() || ! $coupon->is_valid() ) {
			aip_reii_native_checkout_error_v0554( 'Coupon code "' . esc_html( $coupon_code ) . '" is invalid or expired.' );
		}

		$total_cents   = (int) $offer['amount'];
		$discount_type = $coupon->get_discount_type();
		$coupon_amount = (float) $coupon->get_amount();

		if ( 'percent' === $discount_type ) {
			$discount_cents = (int) round( $total_cents * ( $coupon_amount / 100 ) );
		} else {
			$discount_cents = (int) round( $coupon_amount * 100 );
		}
		$final_cents = max( 0, $total_cents - $discount_cents );

		if ( 0 === $final_cents ) {
			// 100% Free Order! Create WooCommerce order directly without Stripe.
			$intake_token = wp_generate_password( 24, false );
			$order        = aip_reii_create_paid_order_v0559(
				$intake,
				$offer,
				'free-coupon-' . wp_generate_password( 12, false ),
				'free-session-' . $intake_token,
				$intake_token,
				'',
				0
			);
			if ( is_wp_error( $order ) ) {
				aip_reii_native_checkout_error_v0554( $order->get_error_message(), 500 );
			}
			$order->apply_coupon( $coupon->get_code() );
			$order->add_order_note( sprintf( 'Order completed for $0.00 using 100%% coupon: %s.', $coupon->get_code() ) );
			$order->save();

			wp_send_json_success( array(
				'checkout_mode' => 'free_order_complete',
				'order_id'      => $order->get_id(),
				'email'         => $email,
			) );
		} else {
			$offer['amount'] = $final_cents;
			$stripe_checkout = aip_reii_prepare_direct_stripe_checkout_v0559( $intake, $product, $offer );
...
function aip_reii_apply_intake_to_item_v0549( $item, $intake, $save = false ) {
	...
	$addon_labels = array(
		'amazon-storefront'  => 'Post to REii’s Amazon Storefront (+$20)',
...
// Never send admin notifications or customer invoice emails for unpaid orders
// (pending, failed, cancelled) before Stripe checkout is completed.
function aip_reii_disable_unpaid_new_order_email_v0579( $recipient, $order ) {
	if ( $order instanceof WC_Order && in_array( $order->get_status(), array( 'pending', 'failed', 'cancelled' ), true ) ) {
		return '';
	}
	return $recipient;
}
add_filter( 'woocommerce_email_recipient_new_order', 'aip_reii_disable_unpaid_new_order_email_v0579', PHP_INT_MAX, 2 );

function aip_reii_disable_unpaid_customer_invoice_v0579( $enabled, $order ) {
...
final class AIP_On_Model_Commerce_GitHub {
	...
	public static function api_orders( $request ) {
		...
		$orders = array_values(
			array_filter(
				$orders,
				function( $order ) use ( $raw_status, $filter_review ) {
					$status = str_replace( 'wc-', '', strtolower( (string) ( $order['status'] ?? '' ) ) );
					$has_deliv = ! empty( $order['deliverables']['images'] ) || ! empty( $order['deliverables']['videos'] );
					$wf_status = $order['workflow_status'] ?? '';
					if ( 'pending' === $status && ! $has_deliv && empty( $order['reference'] ) && empty( $order['uploaded_files'] ) ) {
						return false;
					}
					if ( $filter_review ) {
						return 'content-review' === $wf_status || $has_deliv;
					}
					if ( 'processing' === $raw_status ) {
						return 'processing' === $wf_status && ! $has_deliv;
					}
					return true;
				}
			)
		...
		return rest_ensure_response(
			array(
				'orders' => $orders,
				'total'  => ( $filter_review || 'processing' === $raw_status ) ? count( $orders ) : (int) $result->total,
				'pages'  => ( $filter_review || 'processing' === $raw_status ) ? ( empty( $orders ) ? 0 : 1 ) : (int) $result->max_num_pages,
			)
		);
	}
	...
	public static function api_order_status( $request ) {
		...
		if ( in_array( $status, array( 'content-review', 'content-queued', 'content-creating' ), true ) ) {
			$status = 'processing';
		}
		$order_statuses = wc_get_order_statuses();
		if ( ! array_key_exists( 'wc-' . $status, $order_statuses ) && ! array_key_exists( $status, $order_statuses ) ) {
	...
	public static function api_order_deliver( $request ) {
		...
		// a generated preview for review must keep the order in processing without notifying customer.
		...
		if ( $is_submission ) {
			$order->update_status( 'completed', $status_note, true );
			...
			// Maintain or set order status to processing so it is never dropped into pending
			if ( 'processing' !== $previous_status && 'completed' !== $previous_status ) {
	...
	public static function order_has_storefront_feature( $order ) {
		if ( ! $order instanceof WC_Order ) {
			return false;
		}
		$addon = strtolower( (string) $order->get_meta( '_aip_intake_addon' ) );
		$meta_addon = strtolower( (string) $order->get_meta( '_aip_addon' ) );
		$storefront_meta = strtolower( (string) $order->get_meta( '_aip_amazon_storefront' ) );
		if ( in_array( $storefront_meta, array( 'yes', 'true', '1' ), true ) || 'amazon-storefront' === $addon || 'amazon-storefront' === $meta_addon ) {
			return true;
		}
		if ( method_exists( $order, 'get_items' ) ) {
			foreach ( $order->get_items() as $item ) {
				if ( false !== stripos( $item->get_name(), 'storefront' ) ) {
					return true;
				}
				if ( method_exists( $item, 'get_meta_data' ) ) {
					foreach ( $item->get_meta_data() as $meta ) {
						$k = is_object( $meta ) ? ( $meta->key ?? '' ) : ( $meta['key'] ?? '' );
						$v = is_object( $meta ) ? ( $meta->value ?? '' ) : ( $meta['value'] ?? '' );
						if ( false !== stripos( (string) $k, 'storefront' ) || false !== stripos( (string) $v, 'storefront' ) ) {
							return true;
						}
					}
				}
	...
	public static function email_delivery_links( $order, $sent_to_admin, $plain_text, $email ) {
		...
		$is_storefront = self::order_has_storefront_feature( $order );
		$storefront_url = 'https://www.amazon.com/shop/who_is_leon';
		...
		if ( $plain_text ) {
			...
			if ( $is_storefront ) {
				echo "REII AMAZON STOREFRONT FEATURE\n";
				echo "You chose to feature your video on our official Amazon Storefront. Your video has been uploaded, and you should see it on our storefront in 24 hours:\n";
				echo esc_url_raw( $storefront_url ) . "\n\n";
			}
			return;
		}
		?>
		...
		<div class="aip-email-storefront-card" style="margin:0 0 28px;padding:24px;border:1px solid #d8cef5;border-radius:12px;background:#fbfaff;">
			<small style="color:#6846e6;font-size:10px;font-weight:800;letter-spacing:1.2px;text-transform:uppercase;display:block;margin-bottom:8px;">REii Amazon Storefront Feature</small>
			<h2 style="margin:0 0 8px;font-size:20px;line-height:1.3;color:#1d1827;">Uploaded to REii Storefront</h2>
			<p style="margin:0 0 18px;font-size:14px;line-height:1.5;color:#554c60;">You chose to feature your video on our official Amazon Storefront. Your video has been uploaded, and you should see it on our storefront in <strong>24 hours</strong> as Amazon completes indexing.</p>
			<p style="margin:10px 0 0;">
				<a href="<?php echo esc_url( $storefront_url ); ?>" target="_blank" rel="noopener" style="display:inline-block;background:#5d32ea;color:#ffffff;text-decoration:none;padding:13px 22px;border-radius:8px;font-weight:700;font-size:14px;text-align:center;">View REii Storefront on Amazon &rarr;</a>
			</p>
		</div>
		<?php endif; ?>
		<?php
	...
	public static function passwordless_delivery_request() {
		...
		<style>
		.head{display:none}.reii-library-head{background:#1b1622;border-radius:20px;color:#fff;margin-bottom:22px;padding:34px 38px}.reii-library-head small{color:#bfa8ff;display:block;font-size:10px;font-weight:800;letter-spacing:1.8px;margin-bottom:7px;text-transform:uppercase}.reii-library-head-row{align-items:center;display:flex;gap:24px;justify-content:space-between}.reii-library-head h1{font-size:34px;font-weight:800;letter-spacing:-.8px;margin:0 0 5px}.reii-library-head p{color:#cdc6d5;font-size:14px;margin:0}.reii-library-head-actions{align-items:flex-end;display:flex;flex-direction:column;gap:9px;flex-shrink:0}.reii-library-head-btn{background:var(--purple);border-radius:10px;color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:13.5px;font-weight:750;padding:12px 24px;text-align:center;text-decoration:none;transition:background .15s ease,transform .15s ease;white-space:nowrap;box-shadow:0 4px 16px rgba(104,70,230,.3)}.reii-library-head-btn:hover{background:var(--purple-dark);transform:translateY(-1px)}.upsell{display:none}.delivery-reorder-wrap{display:flex;justify-content:center;margin:18px 0 10px}.delivery-reorder-button{background:var(--purple);border-radius:10px;color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:14px;font-weight:750;padding:14px 34px;text-align:center;text-decoration:none;transition:background .15s ease,transform .15s ease;white-space:nowrap}.delivery-reorder-button:hover{background:var(--purple-dark);transform:translateY(-1px)}.delivery-storefront-card{background:#fbfaff;border:1px solid #d8cef5;border-radius:14px;padding:22px;margin:0 0 24px}.delivery-storefront-kicker{color:var(--purple);display:block;font-size:10px;font-weight:800;letter-spacing:1.2px;margin-bottom:7px;text-transform:uppercase}.delivery-storefront-card h3{color:var(--ink);font-size:18px;font-weight:750;letter-spacing:-.3px;margin:0 0 8px}.delivery-storefront-card p{color:var(--muted);font-size:13.5px;line-height:1.5;margin:0 0 16px}.delivery-storefront-btn{background:#5d32ea;border-radius:8px;color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:13.5px;font-weight:750;padding:11px 20px;text-align:center;text-decoration:none;transition:background .15s ease,transform .15s ease}.delivery-storefront-btn:hover{background:var(--purple-dark);transform:translateY(-1px);color:#fff}@media(max-width:760px){.reii-library-head{border-radius:15px;padding:26px 22px}.reii-library-head-row{align-items:start;flex-direction:column;gap:16px}.reii-library-head h1{font-size:28px}.reii-library-head-actions{align-items:stretch;width:100%;gap:10px;margin-top:4px}.reii-library-head-btn{display:flex;width:100%;padding:13px 20px}.reii-library-head-actions .library-count{text-align:center}.delivery-reorder-wrap{margin:14px 0 8px}.delivery-reorder-button{display:flex;width:100%;padding:13px 20px}.delivery-storefront-card{padding:18px;margin:0 0 20px}.delivery-storefront-btn{display:flex;width:100%;padding:12px 18px}}
		</style>
		<header class="reii-library-head"><small>REIMAGINE · REii AI INFLUENCER</small><div class="reii-library-head-row"><div><h1>Your REii content library</h1><p>Every finished AI influencer UGC video, creative brief, and download in one private place.</p></div><div class="reii-library-head-actions"><a class="reii-library-head-btn" href="<?php echo esc_url( home_url( '/style-by-reii/#submit-project' ) ); ?>">Create another video</a><span class="library-count"><?php echo esc_html( count( $customer_orders ) ); ?> completed order<?php echo 1 === count( $customer_orders ) ? '' : 's'; ?> · <?php echo esc_html( $total_videos ); ?> video<?php echo 1 === $total_videos ? '' : 's'; ?></span></div></div></header>
		<?php foreach ( $customer_orders as $customer_order ) :
			$files = self::delivery_files( $customer_order );
			$details = self::delivery_order_details( $customer_order );
			$is_storefront = self::order_has_storefront_feature( $customer_order );
			$storefront_url = 'https://www.amazon.com/shop/who_is_leon';
			$first_img_key = false;
	...
	private static function addon_catalog() {
		return array(
			'amazon-storefront'  => array( 'label' => 'Post to REii’s Amazon Storefront', 'price' => 20 ),
	...
	public static function filter_completed_email_templates( $located, $template_name, $args, $template_path, $default_path ) {
		...
		if ( 'customer_completed_order' === $email_id ) {
			if ( 'emails/email-addresses.php' === $template_name || 'emails/plain/email-addresses.php' === $template_name ) {
			...
			if ( 'emails/email-order-details.php' === $template_name ) {
				$custom = __DIR__ . '/includes/completed-email-order-details.php';
				if ( file_exists( $custom ) ) {
					return $custom;
				}
			}
			if ( 'emails/plain/email-order-details.php' === $template_name ) {
				$custom = __DIR__ . '/includes/plain-completed-email-order-details.php';
				if ( file_exists( $custom ) ) {
					return $custom;
				}
			}
	...
	public static function customize_email_gettext( $translated_text, $text, $domain ) {
		if ( 'woocommerce' === $domain ) {
			...
			if ( "Here's a reminder of what you've ordered:" === $text || 'Here is a summary of your completed order:' === $text ) {
				if ( self::is_completed_email_rendering() ) {
					return '';
				}
				return 'Here is a summary of your completed order:';
			}
			if ( 'Order summary' === $text && self::is_completed_email_rendering() ) {
				return '';
			}
