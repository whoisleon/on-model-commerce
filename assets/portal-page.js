(function(){
  ...
  function initProductSource(portal){
    ...
    function sync(){
      ...
      syncStorefront(portal);
    }
    portal.addEventListener('change',function(event){
      if(event.target.name==='source-method'||event.target.name==='product-reference')sync();
    });
  ...
  var aipAddons={
    'amazon-storefront':{label:'Post to REii\u2019s Amazon Storefront',price:20},
  ...
  function selectAddon(slug,sourceOrder){
    ...
    if(modal){
      ...
      if(intro)intro.textContent=selected?'Your $30 REii video plus '+selected.label.toLowerCase()+' (+$'+selected.price+'). Add your product details to continue.':'Add an Amazon link or upload your product files. Your $30 order includes one 10-second, AI-created influencer UGC video.';
    }
  }
  ...
  function launchConfetti(container){
    ...
    function render(now){
      ...
      particles.forEach(function(p){
        p.vx*=drag;
        p.vy=Math.min((p.vy*drag)+gravity,1.35);
        p.wobble+=p.wobbleSpeed;
        p.x+=p.vx+Math.sin(p.wobble)*0.45;
        p.y+=p.vy;
        p.rotation+=p.rotationSpeed;
        var fade=progress>0.72?(1-progress)/0.28:1;
        ctx.save();
